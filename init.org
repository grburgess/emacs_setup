...
** Bindings for crux
   #+BEGIN_SRC emacs-lisp
     ...
     ;; crux
   (define-key (current-global-map) [remap move-beginning-of-line] 'crux-move-beginning-of-line)
   (define-key (current-global-map)  (kbd "C-<backspace>") 'crux-kill-line-backwards)
   (define-key (current-global-map)  [remap kill-whole-line] 'crux-kill-whole-line)
   (define-key (current-global-map)  [(shift return)] 'crux-smart-open-line)
   (define-key (current-global-map)  (kbd "C-c I")     'crux-find-user-init-file)
