...
** Bindings for crux
   ...
   ;; crux
   (define-key custom-bindings-map  [remap move-beginning-of-line] 'crux-move-beginning-of-line)
   (define-key custom-bindings-map  (kbd "C-<backspace>") 'crux-kill-line-backwards)
   (define-key custom-bindings-map  [remap kill-whole-line] 'crux-kill-whole-line)
   (define-key custom-bindings-map   [(shift return)] 'crux-smart-open-line)
   (define-key custom-bindings-map  (kbd "C-c I")     'crux-find-user-init-file)
